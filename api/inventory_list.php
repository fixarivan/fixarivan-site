...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (
        ...
        // Короткий список без поиска: не показываем черновики с ведущим «-» и карточки, привязанные к заказу/заявке
        // (в notes есть тег [REQ orderId] — см. order_supply / «под заказ»). По поиску — всё как обычно.
        $sql .= ' AND NOT (
            TRIM(IFNULL(i.name, \'\')) LIKE \'-%\'
            OR TRIM(IFNULL(i.name, \'\')) LIKE \'–%\'
            OR IFNULL(i.notes, \'\') LIKE \'%[REQ %\'
        )';
    }
    if ($sort === 'qty_desc') {
